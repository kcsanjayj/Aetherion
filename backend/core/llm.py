...
class LLMClient:
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            if provider == "openai":
                ...
                if not config.get("api_key"):
                    logger.warning("NVIDIA API key not found, using mock responses")
                    self.client = None
                else:
                    self.client = config["api_key"]  # Store API key for NVIDIA
                    logger.info(f"NVIDIA client initialized with {config.get('available_keys', 1)} API key(s)")
                ...
                if not config.get("api_key"):
                    ...
                    self.client = config["api_key"]  # Store API key for Groq
                    logger.info(f"Groq client initialized with {config.get('available_keys', 1)} API key(s)")
                    
            elif provider == "huggingface":
                if not config.get("api_key"):
                    logger.warning("HuggingFace API key not found, using mock responses")
                    self.client = None
                else:
                    self.client = config["api_key"]  # Store API key for HuggingFace
                    logger.info(f"HuggingFace client initialized with {config.get('available_keys', 1)} API key(s)")
    ...
    async def _generate_nvidia_response(self, prompt: str, temperature: float, max_tokens: int, model: str) -> str:
        ...
        # Ensure API key has nvapi- prefix
        if not api_key.startswith("nvapi-"):
            api_key = f"nvapi-{api_key}"
        
        # Use NVIDIA integrate API endpoint (older but stable)
        ...
        # Use SSL=False to avoid certificate issues
        import ssl
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, json=data, timeout=60, ssl=ssl_context) as response:
                result = await response.json()
                if response.status != 200:
