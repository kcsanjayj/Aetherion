...
async def query_documents(request: QueryRequest):
    ...
    try:
        ...
        # 🎯 Store runtime API key and provider if provided (BYOK mode)
        if request.api_key and request.provider:
            ...
            update_runtime_config({
                ...
                f"{request.provider.upper()}_API_KEY": request.api_key,
            })
            if request.model:
                set_runtime_config(f"{request.provider.upper()}_MODEL", request.model)
            logger.info(f"🔑 Using runtime API key for provider: {request.provider}")
...
async def update_config(config_data: Dict[str, Any]):
    ...
    try:
        ...
        # If no model specified, use default from fallback list (fast, no API call)
        if not model and api_key and provider != "local":
            fallback_models = PROVIDER_FALLBACK_MODELS.get(provider, [])
            if fallback_models:
                model = fallback_models[0]
                logger.info(f"Using default model: {model} for provider: {provider}")

        # Skip blocking API test - will be tested in background
        # Validation happens lazily on first actual query
...
async def list_available_models(request: Dict[str, Any]):
    ...
    try:
        ...
        if provider == "gemini":
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(f"{settings.LOCAL_LLM_URL}/api/tags", timeout=15) as response:
                        if response.status == 200:
                            result = await response.json()
...
async def test_api_connection(test_data: Dict[str, Any]):
    ...
    try:
        ...
        # Create SSL context - disable SSL verification for NVIDIA due to cert issues
        if provider == "nvidia":
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, json=data, timeout=15, ssl=ssl_context) as response:
                if response.status == 200:
                    return {
                        "success": True,
