class AGENTIC_RAG {
    ...
    async sendMessage() {
        ...
        // 🎯 Check if API is configured (clear, non-blocking check)
        if (!this.isApiReady()) {
            this.addMessage('⚠️ Please configure your AI provider API key in AI Config first. Click the ⚠️ Needs Config badge above.', 'assistant');
            return;
        }
    ...
    async sendQueryToBackend(query) {
        try {
            // 🎯 Build request body with session config (BYOK - Bring Your Own Key)
            const requestBody = {
                query: query,
                max_context: 5,
            ...
            // Include API key and provider if configured (sent with EVERY request)
            console.log('🔍 DEBUG sessionConfig:', this.sessionConfig);
            if (this.sessionConfig.apiKey && this.sessionConfig.provider) {
                requestBody.api_key = this.sessionConfig.apiKey;
                requestBody.provider = this.sessionConfig.provider;
                requestBody.model = this.sessionConfig.model;
                console.log('✅ DEBUG: Adding api_key to request:', this.sessionConfig.apiKey.substring(0, 10) + '...');
            } else {
                console.log('❌ DEBUG: api_key missing - sessionConfig:', this.sessionConfig);
            }
    ...
    async initializeApp() {
        try {
            ...
            // 🎯 Set initial clear status - no misleading "API Active"
            this.updateApiStatusBadge(false, '⚠️ Needs Config');
            await this.refreshConfigStatus();
        } catch (error) {
            console.error('Backend health check failed:', error);
            this.updateApiStatusBadge(false, '⚠️ Needs Config');
            this.addMessage('⚠️ Backend connected but API not configured. Please set up your AI provider in AI Config.', 'assistant');
        }
    }
    ...
    updateApiStatusBadge(isActive, text) {
        ...
        // 🎯 Clear status logic - check if API key actually exists in session
        const hasSessionKey = this.sessionConfig && this.sessionConfig.apiKey && this.sessionConfig.provider;
        
        // 🎯 Update hint visibility
        const hint = document.getElementById('apiConfigHint');
        if (hint) {
            hint.style.display = (!hasSessionKey && !isActive) ? 'flex' : 'none';
        }
        
        if (!hasSessionKey && !isActive) {
            // No API key configured at all
            text = text || '⚠️ Needs Config';
            this.apiStatusBadge.classList.remove('api-active', 'api-inactive');
            this.apiStatusBadge.classList.add('api-needs-config');
        } else if (isActive) {
            this.apiStatusBadge.classList.remove('api-inactive', 'api-needs-config');
            this.apiStatusBadge.classList.add('api-active');
            text = text || '✅ API Active';
        } else {
            this.apiStatusBadge.classList.remove('api-active', 'api-needs-config');
            this.apiStatusBadge.classList.add('api-inactive');
            text = text || '❌ API Inactive';
        }
        
        this.apiStatusBadge.textContent = text;
    }
    
    // 🎯 Check if API is ready for queries (fast, non-blocking)
    isApiReady() {
        return !!(this.sessionConfig && this.sessionConfig.apiKey && this.sessionConfig.provider);
    }
    ...
    async startNewChat() {
        // 🎯 Clear frontend session config (memory only)
        this.sessionConfig = {
            apiKey: null,
            provider: null,
            model: null
        };
        console.log('🧹 Frontend session config cleared');
    ...
    async saveConfiguration() {
        ...
        try {
            ...
            if (response.ok) {
                // 🎯 Store in session memory (frontend) - cleared on refresh
                console.log('💾 DEBUG: Saving config - apiKey:', apiKey ? apiKey.substring(0, 10) + '...' : 'null', 'isMasked:', isMaskedKey);
                
                if (apiKey && !isMaskedKey) {
                    // New key entered - store it
                    this.sessionConfig.apiKey = apiKey;
                    this.sessionConfig.provider = provider;
                    this.sessionConfig.model = model;
                    console.log('✅ DEBUG: Stored new API key in sessionConfig');
                } else if (isMaskedKey && this.sessionConfig.provider === provider) {
                    // Key was masked but we're updating same provider - keep existing session config
                    // (sessionConfig already has the key from previous save)
                    this.sessionConfig.provider = provider;
                    this.sessionConfig.model = model || this.sessionConfig.model;
                    console.log('✅ DEBUG: Updated model for existing provider');
                } else if (provider) {
                    // Fallback: set provider even if we don't have key in memory
                    this.sessionConfig.provider = provider;
                    this.sessionConfig.model = model;
                }
                
                console.log('💾 DEBUG: Final sessionConfig:', this.sessionConfig);
                ...
                this.testApiInBackground(provider, apiKey || this.sessionConfig.apiKey, model);
            } else {
                this.updateApiStatusBadge(false, 'API Inactive');
